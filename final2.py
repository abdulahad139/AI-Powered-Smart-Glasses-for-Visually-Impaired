...
class_avg_sizes = {
    ...
    "computer": {"width_ratio": 2.55},
    "Stairs": {"width_ratio": 0.6},
    "mouse": {"width_ratio": 3.0},
    "keyboard": {"width_ratio": 2.8},
    "box": {"width_ratio": 4.0},
    "calculator": {"width_ratio": 2.9},
    "cup": {"width_ratio": 3.2},
    "Glasses": {"width_ratio": 3.5},
    "StudentID_card": {"width_ratio": 4.0},
    "wallet": {"width_ratio": 3.5},
    "Pen": {"width_ratio": 5.0},
}
            ...
            if not pause:
                ...
                if frame_count % 5 == 0:
                    try:
                        ...
                        # Check if object is close enough to announce
                        if distance <= 12.5:  # Within threshold
                            position = get_position(frame.shape[1], cords)
                            print(f"Close object: {label} at {distance:.1f}m on {position}")
                            
                            # Add to queue if not too full
                            if speech_queue.qsize() < 3:  # Allow up to 3 in queue
                                speech_queue.put((label, round(distance, 1), position))
                                print(f"Added {label} to speech queue")
                            else:
                                print(f"Queue full, skipping {label}")
